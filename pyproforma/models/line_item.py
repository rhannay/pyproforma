...
class LineItem:
    ...
    def __init__(
        ...
        if not check_name(name):
            raise ValueError(
                "LineItem name must only contain letters, numbers, underscores, or hyphens (no spaces or special characters)."
            )
        self.name = name
    ...
    def get_value(self, interim_values_by_year: dict, year: int) -> float | None:
        ...
        if year in interim_values_by_year and self.name in interim_values_by_year[year]:
            raise ValueError(
                f"Value for {self.name} in year {year} already exists in interim values."
            )
