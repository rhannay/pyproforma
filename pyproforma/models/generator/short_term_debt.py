...
class ShortTermDebt(Generator):
    ...
    def __init__(
        ...
        if isinstance(interest_rate, dict):
            for year, rate in interest_rate.items():
                if rate < 0:
                    raise ValueError(
                        f"Interest rate for year {year} cannot be negative"
                    )
    ...
    # ----------------------------------
    # MAIN PUBLIC API METHODS (Generator ABC)
    # ----------------------------------
    ...
    def get_values(
        ...
        result[self._debt_outstanding_name] = self._get_debt_outstanding(
    ...
    def _validate_years_not_before_start(self, start_year: int) -> None:
        ...
        if isinstance(self._draws, dict):
            for year in self._draws.keys():
                if year < start_year:
                    raise ValueError(
                        f"Draw year {year} is prior to start year {start_year}"
                    )
        ...
        if isinstance(self._paydown, dict):
            for year in self._paydown.keys():
                if year < start_year:
                    raise ValueError(
                        f"Paydown year {year} is prior to start year {start_year}"
                    )

    def _get_begin_balance(
        self, interim_values_by_year: Dict[int, Dict[str, Any]], year: int = None
    ) -> float:
        ...
        if isinstance(self._begin_balance, (float, int)):
            ...
            raise TypeError(
                f"Begin balance must be a float, not {type(self._begin_balance)}"
            )
    ...
    def _get_draws_for_year(
        ...
        if isinstance(self._draws, dict):
            ...
            if (
                year in interim_values_by_year
                and draws_name in interim_values_by_year[year]
            ):
    ...
    def _get_paydown_for_year(
        ...
        if isinstance(self._paydown, dict):
            ...
            if (
                year in interim_values_by_year
                and paydown_name in interim_values_by_year[year]
            ):
            ...
            raise TypeError(
                f"Paydown must be a dict or string, not {type(self._paydown)}"
            )
    ...
    def _get_debt_outstanding(
        ...
        if isinstance(self._draws, str) or isinstance(self._paydown, str):
            ...
            for check_year in available_years:
                ...
                draws_available = (
                    not isinstance(self._draws, str) or self._draws in year_data
                )
                paydown_available = (
                    not isinstance(self._paydown, str) or self._paydown in year_data
                )
    ...
    def _get_interest(
        ...
        previous_year_balance = self._get_debt_outstanding(
            interim_values_by_year, year - 1
        )
