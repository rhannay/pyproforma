"""
Utilities for handling color values in tables.

Supports CSS color names and hex codes (with or without #).
...
def color_to_hex(color: str) -> str:
    ...
    if suggestions:
        ...
        raise ValueError(
            f"Invalid color: '{color}'. Did you mean: {suggestions_str}?"
        )
    else:
        raise ValueError(
            f"Invalid color: '{color}'. Must be a CSS color name or 6-digit hex code."
        )
