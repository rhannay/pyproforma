...
class TestCharts:
    ...
    def mock_model(self):
        ...
        def mock_li(name):
            ...
            if name in line_item_map:
                ...
                line_item_result.cumulative_percent_change.side_effect = (
                    mock_cumulative_percent_change
                )
    ...
    def test_line_items_multiple_chart(self, charts, mock_model):
        ...
        with patch.object(Chart, "to_plotly") as mock_to_plotly:
            ...
            result = charts.line_items(
                ["revenue", "expenses"], width=900, height=500, template="plotly"
            )
    ...
    def test_cumulative_percent_change_single_item(self, charts, mock_model):
        ...
        with patch.object(Chart, "to_plotly") as mock_to_plotly:
            ...
            result = charts.cumulative_percent_change(
    ...
    def test_item_chart_creation_details(
        ...
        mock_dataset_class.assert_called_once_with(
            label="Revenue", data=[100.0, 150.0, 200.0], color="blue", type="bar"
        )
    ...
    @patch("pyproforma.charts.charts.Chart")
    @patch("pyproforma.charts.charts.ChartDataSet")
    def test_items_chart_creation_details(
        self, mock_dataset_class, mock_chart_class, charts, mock_model
    ):
        ...
        mock_dataset_class.assert_any_call(
            label="Revenue", data=[100.0, 150.0, 200.0], type="line"
        )
        mock_dataset_class.assert_any_call(
            label="Expenses", data=[50.0, 75.0, 100.0], type="line"
        )
    ...
    @patch("pyproforma.charts.charts.Chart")
    @patch("pyproforma.charts.charts.ChartDataSet")
    def test_cumulative_percent_change_chart_creation_details(
        self, mock_dataset_class, mock_chart_class, charts, mock_model
    ):
