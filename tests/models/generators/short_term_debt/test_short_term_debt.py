...
class TestShortTermDebtBasic:
    ...
    def test_defined_names(self):
        """Test that defined_names returns the correct field names."""
        debt = ShortTermDebt(
            name="test_debt",
            draws={2020: 100000},
            paydown={2021: 50000},
    ...
    def test_negative_interest_rate_dict_validation(self):
        ...
        with pytest.raises(
            ValueError, match="Interest rate for year .* cannot be negative"
        ):
            ShortTermDebt(
                name="test_debt",
                draws={},
    ...
    def test_negative_draw_validation(self):
        ...
        with pytest.raises(
            ValueError, match="Draw amount for year .* cannot be negative"
        ):
            ShortTermDebt(
                name="test_debt",
                draws={2020: -100000},
    ...
    def test_negative_paydown_validation(self):
        ...
        with pytest.raises(
            ValueError, match="Paydown amount for year .* cannot be negative"
        ):
            ShortTermDebt(
                name="test_debt",
                draws={},
...
class TestShortTermDebtFixedParameters:
    ...
    def test_simple_debt_no_activity(self):
        ...
        assert values_2020["debt_outstanding"] == 1000000
        assert values_2020["draw"] == 0.0
        assert values_2020["principal"] == 0.0
        assert values_2020["interest"] == 50000  # 1M * 0.05

        # Test year 2021 (no activity, same balance)
        values_2021 = debt.get_values(interim_values, 2021)
        assert values_2021["debt_outstanding"] == 1000000
        assert values_2021["draw"] == 0.0
        assert values_2021["principal"] == 0.0
        assert values_2021["interest"] == 50000  # 1M * 0.05
    ...
    def test_debt_with_draws_only(self):
        ...
        values_2020 = debt.get_values(interim_values, 2020)
        assert values_2020["debt_outstanding"] == 700000  # 500k + 200k
        assert values_2020["draw"] == 200000
        assert values_2020["principal"] == 0.0
        assert _is_close(values_2020["interest"], 30000)  # 500k * 0.06

        # Test year 2021 (second draw)
        values_2021 = debt.get_values(interim_values, 2021)
        assert values_2021["debt_outstanding"] == 1000000  # 700k + 300k
        assert values_2021["draw"] == 300000
        assert values_2021["principal"] == 0.0
        assert _is_close(values_2021["interest"], 42000)  # 700k * 0.06
    ...
    def test_debt_with_paydowns_only(self):
        ...
        values_2020 = debt.get_values(interim_values, 2020)
        assert values_2020["debt_outstanding"] == 900000  # 1M - 100k
        assert values_2020["draw"] == 0.0
        assert values_2020["principal"] == 100000
        assert _is_close(values_2020["interest"], 40000)  # 1M * 0.04

        # Test year 2021 (second paydown)
        values_2021 = debt.get_values(interim_values, 2021)
        assert values_2021["debt_outstanding"] == 750000  # 900k - 150k
        assert values_2021["draw"] == 0.0
        assert values_2021["principal"] == 150000
        assert _is_close(values_2021["interest"], 36000)  # 900k * 0.04
    ...
    def test_debt_with_draws_and_paydowns(self):
        ...
        values_2020 = debt.get_values(interim_values, 2020)
        assert values_2020["debt_outstanding"] == 1500000  # 1M + 500k
        assert values_2020["draw"] == 500000
        assert values_2020["principal"] == 0.0
        assert _is_close(values_2020["interest"], 50000)  # 1M * 0.05

        # Test year 2021 (draw and paydown)
        values_2021 = debt.get_values(interim_values, 2021)
        assert (
            values_2021["debt_outstanding"] == 1600000
        )  # 1.5M + 300k - 200k
        assert values_2021["draw"] == 300000
        assert values_2021["principal"] == 200000
        assert _is_close(values_2021["interest"], 75000)  # 1.5M * 0.05

        # Test year 2022 (paydown only)
        values_2022 = debt.get_values(interim_values, 2022)
        assert values_2022["debt_outstanding"] == 1000000  # 1.6M - 600k
        assert values_2022["draw"] == 0.0
        assert values_2022["principal"] == 600000
        assert _is_close(values_2022["interest"], 80000)  # 1.6M * 0.05
    ...
    def test_year_before_start_year(self):
        ...
        assert values_2020["debt_outstanding"] == 1300000  # 1000k + 300k
        assert values_2020["draw"] == 300000
        assert values_2020["principal"] == 0.0
...
class TestShortTermDebtDynamicParameters:
    ...
    def test_dynamic_interest_rate(self):
        ...
        values_2020 = debt.get_values(interim_values, 2020)
        assert values_2020["debt_outstanding"] == 1000000
        assert _is_close(values_2020["interest"], 30000)  # 1M * 0.03

        # Test year 2021 with 4.5% rate
        values_2021 = debt.get_values(interim_values, 2021)
        assert values_2021["debt_outstanding"] == 1000000
        assert _is_close(values_2021["interest"], 45000)  # 1M * 0.045
    ...
    def test_fixed_begin_balance(self):
        ...
        values_2020 = debt.get_values(interim_values, 2020)
        assert values_2020["debt_outstanding"] == 750000
        assert _is_close(values_2020["interest"], 37500)  # 750k * 0.05
    ...
    def test_dynamic_draws_and_paydown(self):
        ...
        assert values_2020["draw"] == 200000
        assert values_2020["principal"] == 0.0
        assert _is_close(values_2020["interest"], 20000)  # 500k * 0.04

        # Test year 2021
        values_2021 = debt.get_values(interim_values, 2021)
        assert values_2021["debt_outstanding"] == 650000  # 700k + 100k - 150k
        assert values_2021["draw"] == 100000
        assert values_2021["principal"] == 150000
        assert _is_close(values_2021["interest"], 28000)  # 700k * 0.04
    ...
    def test_interest_rate_dict(self):
        ...
        interim_values = {2020: {}, 2021: {}, 2022: {}}

        # Test year 2020 - should use 4% rate
        values_2020 = debt.get_values(interim_values, 2020)
        assert (
            values_2020["debt_outstanding"] == 600000
        )  # 500k + 100k
        assert values_2020["draw"] == 100000
        assert values_2020["principal"] == 0.0
        assert _is_close(
            values_2020["interest"], 20000
        )  # 500k * 0.04

        # Test year 2021 - should use 5% rate
        values_2021 = debt.get_values(interim_values, 2021)
        assert (
            values_2021["debt_outstanding"] == 620000
        )  # 600k + 50k - 30k
        assert values_2021["draw"] == 50000
        assert values_2021["principal"] == 30000
        assert _is_close(
            values_2021["interest"], 30000
        )  # 600k * 0.05

        # Test year 2022 - should use 6% rate
        values_2022 = debt.get_values(interim_values, 2022)
        assert (
            values_2022["debt_outstanding"] == 500000
        )  # 620k + 0 - 120k
        assert values_2022["draw"] == 0.0
        assert values_2022["principal"] == 120000
        assert _is_close(
            values_2022["interest"], 37200
        )  # 620k * 0.06
    ...
    def test_interest_rate_dict_missing_year_error(self):
        ...
        with pytest.raises(
            ValueError,
            match="Interest rate for year 2021 not found in interest rate dictionary",
        ):
    ...
    def test_missing_dynamic_parameter_error(self):
        ...
        with pytest.raises(
            ValueError, match="Could not find interest rate 'missing_rate'"
        ):
    ...
    def test_none_dynamic_parameter_error(self):
        ...
        with pytest.raises(
            ValueError, match="Interest rate 'none_rate' for year .* is None"
        ):
            debt.get_values(interim_values, 2020)
