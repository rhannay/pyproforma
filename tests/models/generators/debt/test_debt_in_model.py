...
class TestDebtParamsFromValueMatrix:
    ...
    def test_debt_par_amounts_from_value_matrix(self):
        ...
        assert _is_close(model.generator("test_from_matrix").field("principal", 2020), p1)
        assert _is_close(model.generator("test_from_matrix").field("interest", 2020), i1)
        assert model.generator("test_from_matrix").field("bond_proceeds", 2020) == 1_000_000
        ...
        assert _is_close(model.generator("test_from_matrix").field("principal", 2021), p1 + p2)
        assert _is_close(model.generator("test_from_matrix").field("interest", 2021), i1 + i2)
        assert model.generator("test_from_matrix").field("bond_proceeds", 2021) == 1_500_000
        ...
        assert _is_close(model.generator("test_from_matrix").field("principal", 2022), p1 + p2)
        assert _is_close(model.generator("test_from_matrix").field("interest", 2022), i1 + i2)
        assert model.generator("test_from_matrix").field("bond_proceeds", 2022) == 0
    ...
    def test_all_debt_params_from_value_matrix(self):
        ...
        p1, i1 = _get_p_i(i=0.05, p=1_000_000, t=30, sy=2020, y=2020)
        assert _is_close(model.generator("test_all_params").field("principal", 2020), p1)
        assert _is_close(model.generator("test_all_params").field("interest", 2020), i1)
        assert model.generator("test_all_params").field("bond_proceeds", 2020) == 1_000_000
        ...
        # Total values should be sum of both issuances
        assert _is_close(model.generator("test_all_params").field("principal", 2021), p1 + p2)
        assert _is_close(model.generator("test_all_params").field("interest", 2021), i1 + i2)
        assert model.generator("test_all_params").field("bond_proceeds", 2021) == 1_500_000
        ...
        # Total values should be sum of both issuances, with no new bond proceeds
        assert _is_close(model.generator("test_all_params").field("principal", 2022), p1 + p2)
        assert _is_close(model.generator("test_all_params").field("interest", 2022), i1 + i2)
        assert model.generator("test_all_params").field("bond_proceeds", 2022) == 0
        ...
        model = Model(
            line_items=[
                ...
                LineItem(
                    name="par_amount_2",
                    category="debt_params",
                    formula="par_amount",  # Formula that simply returns par_amount's value  # noqa: E501
                ),
                # Second formula layer - references the first formula layer
                LineItem(
                    name="par_amount_3",
                    category="debt_params",
                    formula="par_amount_2",  # Formula that references the first formula layer  # noqa: E501
                ),
                # Third formula layer - references the second formula layer
                LineItem(
                    name="par_amount_4",
                    category="debt_params",
                    formula="par_amount_3",  # Formula that references the second formula layer  # noqa: E501
                ),
            ],
            years=years,
            generators=[debt],  # Add the debt generator directly to the model
