...
class TestModelValidateFormulas:
    ...
    def test_valid_formulas_pass_validation(self):
        ...
        profit = LineItem(
            name="profit", category="income", formula="revenue - expenses"
        )
    ...
    def test_invalid_formula_raises_error(self):
        ...
        assert (
            "Formula contains undefined line item names: unknown_variable" in error_msg
        )
    ...
    def test_time_offset_formula_validation(self):
        ...
        revenue = LineItem(
            name="revenue", category="income", values={2023: 1000, 2024: 1100}
        )
        ...
        try:
            ...
            )  # Only 2024 to avoid runtime issues
        ...
        invalid_growth = LineItem(
            name="invalid_growth",
            category="metrics",
            formula="revenue - unknown_var[-1]",
        )
    ...
    def test_empty_and_none_formulas_ignored(self):
        ...
        revenue = LineItem(name="revenue", category="income", values={2023: 1000})
        empty_formula = LineItem(
            name="empty", category="expense", formula="", values={2023: 100}
        )
        none_formula = LineItem(
            name="none_item", category="expense", formula=None, values={2023: 50}
        )
        whitespace_formula = LineItem(
            name="whitespace", category="expense", formula="   ", values={2023: 25}
        )
    ...
    def test_formula_validation_called_in_recalculate(self):
        ...
        invalid_item = LineItem(
            name="invalid", category="expense", formula="unknown_var * 2"
        )
    ...
    def test_complex_formula_validation(self):
        ...
        # Should not raise any exception
        model = Model(
            line_items=[revenue, cogs, opex, tax_rate, net_income], years=[2023]
        )

        # Verify it calculates correctly
        assert (
            model.value("net_income", 2023) == 225.0
        )  # (1000 - 400 - 300) * (1 - 0.25)
