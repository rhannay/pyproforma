...
def test_update_multiple_line_items_formulas(basic_model):
    ...
    assert basic_model.get_line_item_definition("costs").formula == "revenue * 0.6"
    assert basic_model["costs", 2023] == 60  # 100 * 0.6 = 60

    assert basic_model.get_line_item_definition("margin").formula == "profit / revenue * 100"
    assert basic_model["margin", 2023] == 40  # (100-60)/100 * 100 = 40
    assert basic_model.get_line_item_definition("margin").value_format == "no_decimals"
...
def test_update_multiple_line_items_invalid_formula(basic_model):
    ...
    # Check that no changes were applied (transaction rolled back)
    assert basic_model["revenue", 2023] == 100
    assert basic_model.get_line_item_definition("profit").formula == "revenue - costs"
...
def test_update_multiple_line_items_change_category(basic_model):
    ...
    assert basic_model.get_line_item_definition("revenue").category == "profit"
    assert basic_model.get_line_item_definition("costs").category == "profit"
...
def test_update_multiple_line_items_all_properties(basic_model):
    ...
    updated_item = basic_model.get_line_item_definition("total_revenue")
...
def test_update_multiple_line_items_mixed_updates(basic_model):
    ...
    assert basic_model["revenue", 2023] == 200
    assert basic_model.get_line_item_definition("revenue").label == "Annual Revenue"

    assert basic_model.get_line_item_definition("costs").formula == "revenue * 0.5"
    assert basic_model["costs", 2023] == 100  # 200 * 0.5

    assert basic_model.get_line_item_definition("profit").value_format == "two_decimals"
