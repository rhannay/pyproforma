...
class TestValidateExistingDebtService:
    ...
    def test_valid_single_entry(self):
        ...
        valid_data = [{"year": 2024, "principal": 1000.0, "interest": 50.0}]
        # Should not raise any exception
    ...
    def test_invalid_not_list(self):
        ...
        with pytest.raises(
            ValueError, match="existing_debt_service must be a list of dictionaries"
        ):
    ...
    def test_invalid_entry_not_dict(self):
        ...
        with pytest.raises(
            ValueError, match="existing_debt_service entry 1 must be a dictionary"
        ):
    ...
    def test_invalid_missing_year_key(self):
        ...
        with pytest.raises(
            ValueError,
            match="existing_debt_service entry 0 is missing required keys: {'year'}",
        ):
            Debt._validate_existing_debt_service(invalid_data)
    ...
    def test_invalid_missing_multiple_keys(self):
        ...
        with pytest.raises(
            ValueError, match="existing_debt_service entry 0 is missing required keys"
        ):
            Debt._validate_existing_debt_service(invalid_data)

    def test_invalid_year_not_integer(self):
        """Test that non-integer year raises ValueError."""
        invalid_data = [{"year": 2024.5, "principal": 1000.0, "interest": 50.0}]
        with pytest.raises(
            ValueError, match="existing_debt_service entry 0: 'year' must be an integer"
        ):
            Debt._validate_existing_debt_service(invalid_data)

        invalid_data = [{"year": "2024", "principal": 1000.0, "interest": 50.0}]
        with pytest.raises(
            ValueError, match="existing_debt_service entry 0: 'year' must be an integer"
        ):
            Debt._validate_existing_debt_service(invalid_data)
    ...
    def test_invalid_principal_not_number(self):
        ...
        invalid_data = [{"year": 2024, "principal": "1000", "interest": 50.0}]
        with pytest.raises(
            ValueError,
            match="existing_debt_service entry 0: 'principal' must be a number",
        ):
            Debt._validate_existing_debt_service(invalid_data)

    def test_invalid_interest_not_number(self):
        """Test that non-numeric interest raises ValueError."""
        invalid_data = [{"year": 2024, "principal": 1000.0, "interest": "50"}]
        with pytest.raises(
            ValueError,
            match="existing_debt_service entry 0: 'interest' must be a number",
        ):
            Debt._validate_existing_debt_service(invalid_data)
    ...
    def test_invalid_negative_principal(self):
        ...
        with pytest.raises(
            ValueError,
            match="existing_debt_service entry 0: 'principal' must be non-negative",
        ):
            Debt._validate_existing_debt_service(invalid_data)
    ...
    def test_invalid_negative_interest(self):
        ...
        with pytest.raises(
            ValueError,
            match="existing_debt_service entry 0: 'interest' must be non-negative",
        ):
            Debt._validate_existing_debt_service(invalid_data)
    ...
    def test_invalid_duplicate_years(self):
        ...
        with pytest.raises(
            ValueError,
            match="existing_debt_service years must be provided in ascending order",
        ):
            Debt._validate_existing_debt_service(invalid_data)
