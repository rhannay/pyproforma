...
class TestCalculateDebtOutstandingForIssue:
    ...
    def test_single_payment_outstanding(self):
        ...
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2024) == 1000.0

        # On the payment year, outstanding is 0 (payment is made at end of year)
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2025) == 0.0

        # After the payment year, outstanding is 0
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2026) == 0.0
    ...
    def test_multiple_payments_outstanding(self):
        ...
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2023) == 2600.0  # 500 + 600 + 700 + 800

        # After first payment: remaining payments are outstanding
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2024) == 2100.0  # 600 + 700 + 800

        # After second payment: remaining payments are outstanding
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2025) == 1500.0  # 700 + 800

        # After third payment: remaining payments are outstanding
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2026) == 800.0   # 800

        # After all payments: no outstanding debt
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2027) == 0.0
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2028) == 0.0
    ...
    def test_zero_principal_payments(self):
        ...
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2023) == 1000.0
    ...
    def test_decimal_principal_payments(self):
        ...
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2023) == pytest.approx(1000.0, rel=1e-2)
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2024) == pytest.approx(666.67, rel=1e-2)
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2025) == pytest.approx(333.34, rel=1e-2)
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2026) == 0.0
    ...
    def test_edge_case_year_boundaries(self):
        ...
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 1900) == 1000.0
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2100) == 0.0

        # Test with exact payment year
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2024) == 0.0
    ...
    def test_large_debt_amounts(self):
        ...
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2023) == 3_000_000.0
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2024) == 2_000_000.0
        assert Debt._calculate_debt_outstanding_for_issue(debt_service_schedule, 2025) == 0.0
...
class TestGetDebtOutstanding:
    ...
    def test_new_debt_issues_only(self):
        ...
        assert outstanding_2025 > outstanding_2024  # More debt outstanding after second issue
    ...
    def test_existing_and_new_debt_combined(self):
        ...
        expected_new_debt = debt._calculate_debt_outstanding_for_issue(debt.ds_schedules[2025], 2025)
    ...
    def test_debt_outstanding_progression(self):
        ...
        assert outstanding_2023 > outstanding_2026  # Should definitely be less at the end
    ...
    def test_zero_par_amount_years(self):
        ...
        expected = (debt._calculate_debt_outstanding_for_issue(debt.ds_schedules[2024], 2026) +
