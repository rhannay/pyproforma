...
def test_lineitem_formula_init_with_label():
    f = LineItem(name="test", category="calculated", formula="a + b", label="Test Label")
    assert f.name == "test"
...
def test__calculate_formula_with_invalid_formulas():
    ...
    assert result == 10 + 5 * 2  # should respect order of operations (multiplication before addition)
    # Parentheses
    result = calculate_formula('(a + b) * 2', values_matrix, 2024)
